...
let
  ...
  workspaceSwitching = builtins.foldl' (x: y: x // y) { } (
    lib.genList (
      x:
      let
        ws = toString (x + 1);
      in
      {
        "${mod} + ${ws}".action.dispatch.focus = [ { workspace = ws; } ];
        "${mod} + SHIFT + ${ws}".action.dispatch.window.move = [ { workspace = ws; } ];
      }
    ) 9
  );
...
util.mkModule {
  ...
  homeManager = {
    gipphe.programs.hyprland = {
      settings.bind = workspaceSwitching // {
        # Close current window
        "${mod} + Q".action.dispatch.exec_cmd = [ "${killactive}" ];
        # Force close current window
        "${mod} + SHIFT + Q".action.dispatch.window.kill = [ ];
      ...
      submaps.locked.settings.bind."${mod} + ALT_L + H".action.dispatch.submap = [ "reset" ];
    };
  };
}
