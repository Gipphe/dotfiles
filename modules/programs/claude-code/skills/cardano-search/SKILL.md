...
allowed-tools: Read, Grep, Glob, Bash(ls *), Bash(grep *), Agent(Explore)
---

All the repositories in the [repos](repos) directory are Haskell repositories
pertaining to the Cardano and Ouroboros ecosystem of Haskell packages. Some of
the directories for the repositories might be symlinks to the repository
elsewhere in the system, which should be taken into consideration when using
bash tools like `find`.

Most, if not all, of them are multi-package repositories. Each package has a
corresponding `<name>.cabal` file that describes the package, including
