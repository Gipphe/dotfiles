{ lib, util, ... }:
let
  inherit (builtins) isString;
  inherit (import ./util.nix { inherit lib; }) mkPowershellInline profileOpt toPowershell powershellInlineType;
  defaultEnvs = {
    XDG_CONFIG_HOME = mkPowershellInline ''"$($Env:USERPROFILE)\\.config"'';
  };
  order = import ./order.nix;
in
util.mkToggledModule [ "windows" ] {
  name = "environment";

  options.gipphe.windows.profiles = profileOpt {
    environment = {
      variables = lib.mkOption {
        description = "Environment variables";
        type =
          with lib.types;
          attrsOf (oneOf [
            str
            (submodule {
              options = {
                enable = lib.mkOption {
                  description = "Whether to set the environment variable or not.";
                  type = bool;
                  default = true;
                };
                value = lib.mkOption {
                  description = "Value of the environment variable";
                  type = oneOf [
                    str
                    powershellInlineType
                  ];
                };
              };
            })
          ]);
        default = { };
      };
    };
  };

  hm.gipphe.windows.powershell-script =
    lib.mkOrder order.env # powershell
      ''
        class Env {
          [PSCustomObject]$Logger

          Env([PSCustomObject]$Logger) {
            $this.Logger = $Logger
          }
          [Void] Install() {
            $this.Logger.Info(" Setting env vars...")
            $ChildLogger = $this.Logger.ChildLogger()
            $EnvVars = $Profile.environment.variables
            $DefaultVars = ${toPowershell defaultEnvs}
            $EnvVars.GetEnumerator() | ForEach-Object {
              $key = $_.Key
              $val = $_.Value
              if ($val.GetType().Name -ne "String") {
                if (-not $val.enable) {
                  $this.Logger.Info(" Skipping disabled env var $key.")
                  return
                }
                $val = $_.Value.value
              }
              [Environment]::SetEnvironmentVariable($key, $val, 'User')
              $ChildLogger.Info(" $key env var set")
            }
            $this.Logger.Info(" Env vars set.")
          }
        }
        $Env = [Env]::new($Logger)
        $Env.Install()
      '';
}
