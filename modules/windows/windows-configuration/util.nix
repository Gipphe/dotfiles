  ...
  toPowershell =
    ...
    let
      ...
      isPowershellInline =
        ...
        _type == "powershell-inline";

      generatedBindings =
        assert assertMsg (badVarNames == [ ]) "Bad Powershell var names: ${toPretty { } badVarNames}";
        assert assertMsg (collidingNames == [ ])
          "Name collisions in Powershell var names: ${toPretty { } (map (n: n.name) collidingNames)}";
        concatStrings (
      ...
      collidingNames = filter (
        { lowered, ... }: length (filter (x: x.lowered == lowered) loweredNames) > 1
      ) loweredNames;
    in
    if asBindings then
      generatedBindings
      ...
      if isPowershellInline v then
        ...
        "@{${introSpace}${
          concatItems (mapAttrsToList (key: value: ''"${toJSON key}" = ${toPowershell innerArgs value}'') v)
        }${outroSpace}}"
    else
